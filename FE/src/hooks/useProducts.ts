...
export const useProducts = () => {
  ...
  const createNewProduct = async (data: { title: string; price: string; image_url: string }) => {
  ...
  const fetchProducts = useCallback(async (limit?: number, page?: number) => {
    const result = await dispatch(getProducts({ limit, page }));
    return result.type.endsWith('fulfilled');
  }, [dispatch]);

  const updateExistingProduct = async (id: string, data: { title?: string; price?: string; image_url?: string }) => {
